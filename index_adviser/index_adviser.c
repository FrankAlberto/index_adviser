...
#define IND_ADV_TABL "advise_index"
					...
					ParamListInfo	boundParams)
{
	...
	PlannedStmt *actual_plan;
	PlannedStmt *new_plan;
	static int8	SuppressRecursion = 0;
	...
	elog(DEBUG3,"IND ADV: Returning new_plan from planner_callback()");
 ...
 * 									(	Query *query,-------------ok
										IntoClause *into,
										ExplainState *es,
										const char *queryString,--ok
										ParamListInfo params)-----ok
 */
static void
ExplainOneQuery_callback(	Query			*query,//-------ok
							IntoClause *into,
							ExplainState *es,
							const char		*queryString,//-ok
							ParamListInfo	params)//-------ok

{
	...
	elog(DEBUG3,"IND ADV: Returning new_plan from ExplainOneQuery_callback()");
	...
	if ( new_plan )
	{
	    ...
	    appendStringInfo(es->str, "** Plan with hypothetical indexes ** \n");
		ExplainOnePlan( new_plan, into, es, queryString, params );
	...
	elog(DEBUG4,"IND ADV: Index Candidates Destroyed ");
	...
	while(cell != NULL)
	{
			...
			|| IsSystemRelation(base_rel))
		{
			...
			elog( DEBUG1,
					...
					base_rel_oid );
			elog(DEBUG1, " Unsuported relation condition1(%d) condition2(%d)",
					base_rel->rd_backend,
					IsSystemRelation(base_rel) );
		...
		case T_BoolExpr:
		{
			...
			else
			{
				...
				foreach( cell, expr->args )
				{
					...
					compositeCandidates = merge_candidates(compositeCandidates,cicList);
				}
		...
		case T_Var:
		{
			...
			if( rte->rtekind == RTE_RELATION )
			{
					...
					&& base_rel->rd_rel->reltuples > 1 )
				{
					...
					for( i = 1; i < INDEX_MAX_KEYS; ++i ) cand->collationOid[i] = expr->varcollid;//adding collation Oid to index Candidate  
                                        
					for( i = 1; i < INDEX_MAX_KEYS; ++i ) cand->collationOid[i] = InvalidOid;
					cand->reloid        = rte->relid;
					cand->idxused       = false;

					for( i = 0; i < base_rel->rd_att->natts; i++){
						Form_pg_attribute att = base_rel->rd_att->attrs[i];
						indexColNames = lcons(att->attname.data,indexColNames);
					}

					cand->indexColNames = indexColNames;
					...
					*/
					if( ( cand1->ncols + cand2->ncols ) < INDEX_MAX_KEYS )
					{
						...
						if( !foundCommon )
						{
							...
							cic1->idxused		= false;
                                                        cic1->indexColNames = list_concat_unique(cand1->indexColNames, cand2->indexColNames);
							...
							cic2->idxused		= false;          
                                                        cic2->indexColNames = list_concat_unique(cand2->indexColNames, cand1->indexColNames);
							...
							for( i1 = 0; i1 < cand1->ncols; ++i1)
							{
								...
								cic1->varattno[ i1 ]
									...
									= cand1->varattno[ i1 ];
                                                                cic1->collationOid[i1]
                                                                        = cic2->collationOid[cand2->ncols + i1]
                                                                        = cand1->collationOid[i1];                                                                
							}
							...
							for( i1 = 0; i1 < cand2->ncols; ++i1)
							{
								...
								cic1->varattno[cand1->ncols + i1]
									= cic2->varattno[ i1 ]
									= cand2->varattno[ i1 ];
                                                                cic1->collationOid[cand1->ncols + i1] 
                                                                        = cic2->collationOid[i1]
                                                                        = cand2->collationOid[i1];
							}                                                        
...
create_virtual_indexes( List* candidates )
{
			...
			cell = next)
	{
		...
		indexInfo->ii_NumIndexAttrs = cand->ncols;
                 ...
                 elog(DEBUG1, "IND ADV: comienza subscilo ............");
		for( i = 0; i < cand->ncols; ++i )
		{
			/* prepare op_class[] */
			...
			indexInfo->ii_KeyAttrNumbers[i] = cand->varattno[i];
                ...
                elog(DEBUG1, "IND ADV: subscilo terminado..............");
		/* if we decided not to create the index above, try next candidate */
		if( i < cand->ncols )
		{
			candidates = list_delete_cell( candidates, cell, prev );
			elog(DEBUG1, "IND ADV: indexInfo destroyed" );
			continue;
		}
		...
		/*
		 *   Relation heapRelation,					Oid heapRelationId,
		 ...
		 */
		Relation heapRelation =	RelationIdGetRelation(cand->reloid);
		elog( DEBUG2, "IND ADV: creating virtual index name=%s  \n whith indexColName(%d) \n"
					  " whith collationOid (%d)", idx_name, cand->indexColNames->length , cand->collationOid[0] );

		cand->idxoid = index_create( 	heapRelation,
										idx_name,
										...
										cand->collationOid,//collationObjectId
										op_class,
										NULL,
										(Datum)0,
										false,
		...
		elog( DEBUG2, "IND ADV: virtual index created: oid=%d name=%s size=%d",
...
/*
		IndexCandidate* cand = (IndexCandidate*)lfirst( cell );
		// revisar esta función y ver si se puede utilizar para poder limpiar los índices///
		index_set_state_flags(cand->idxoid,INDEX_DROP_SET_DEAD);
